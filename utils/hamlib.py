...
def Heisenberg(n,J,periodic=True):
    ...
    if periodic:
        ...
        for j in range(n-1):
            ...
            ops[2*n+j][j]='Y'
            ops[2*n+j][(j+1)%(n)]='Y'
        coeff = [[J] for i in range(3*(n-1))]
        # assume h to be 1
        for k in range(n):
            coeff.append([1])
            if k!=n-1:
                ops[3*n+k][k] = 'Z'
    H = [[''.join(op)] for op in ops]
    H_grouped = [[op[0] for op in H[:n]+H[3*n:]],[op[0] for op in H[n:2*n]], [op[0] for op in H[2*n:3*n]]]
    coeff_grouped = [[co[0] for co in coeff[:n]+coeff[3*n:]],[co[0] for co in coeff[n:2*n]], [co[0] for co in coeff[2*n:3*n]]]
    ...
    return H_matrix, H, coeff, H_grouped, coeff_grouped
